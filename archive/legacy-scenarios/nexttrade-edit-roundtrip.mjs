...
// render, revert. Covers products — the entity type added in this phase.
// V3.7.2 Phase 4: the test also snapshots and restores ownership metadata
// (manualModifiedAt / sourceType) — a roundtrip must not leave rows marked
// as manually edited.
...
const SITE = process.env.SITE_ID || 'cmubop32l00018n6ipq55zzo1';
const TOKEN = process.env.PREVIEW_TOKEN || 'mubop32v09uy';
const M = 'V37EDIT';
const results = [];
...
const orig = {
  ...
  prodSummary: prod.summary,
  newsTitle: newsItem.title,
  phone: before.site.siteSettings.phone,
  menuLabel: menuItem.label,
};

// Ownership metadata snapshot — restored verbatim after the reverts so the
// test never leaves rows looking manually edited.
const ownershipOrig = {
  page: { manualModifiedAt: home.manualModifiedAt ?? null, sourceType: home.sourceType ?? null },
  service: { manualModifiedAt: svc.manualModifiedAt ?? null, sourceType: svc.sourceType ?? null },
  project: { manualModifiedAt: proj.manualModifiedAt ?? null, sourceType: proj.sourceType ?? null },
  product: { manualModifiedAt: prod.manualModifiedAt ?? null, sourceType: prod.sourceType ?? null },
  news: { manualModifiedAt: newsItem.manualModifiedAt ?? null, sourceType: newsItem.sourceType ?? null },
};
const restoreOwnership = async () => {
  const set = (o) => ({ manualModifiedAt: o.manualModifiedAt ? new Date(o.manualModifiedAt) : null });
  await prisma.page.update({ where: { id: home.id }, data: set(ownershipOrig.page) });
  await prisma.service.update({ where: { id: svc.id }, data: set(ownershipOrig.service) });
  await prisma.project.update({ where: { id: proj.id }, data: set(ownershipOrig.project) });
  await prisma.product.update({ where: { id: prod.id }, data: set(ownershipOrig.product) });
  await prisma.newsPost.update({ where: { id: newsItem.id }, data: set(ownershipOrig.news) });
  const ss = await prisma.siteSettings.findFirst({ where: { siteId: SITE } });
  if (ss) await prisma.siteSettings.update({ where: { id: ss.id }, data: { manualModifiedAt: null } });
};
...
const edits = [
  ...
  ['product title', () => api('PUT', `/api/cms/sites/${SITE}/products/${prod.id}`, { title: `${M} ${orig.prodTitle}` })],
...
const prodPath = prod.slug;
const prodDetail = await showcaseText(prodPath);
check('product detail renders edit', prodDetail.includes(`${M} ${orig.prodTitle}`), `route /${prodPath}`);
...
check('menu label renders edit', homeText.toLowerCase().includes(`${M} ${orig.menuLabel}`.toLowerCase()), 'nav label');
...
const reverts = [
  ...
  ['product title', () => api('PUT', `/api/cms/sites/${SITE}/products/${prod.id}`, { title: orig.prodTitle })],
...
// Restore ownership metadata the PUT endpoints set (manualModifiedAt) back to
// its pre-test value, then verify no residue remains.
await restoreOwnership();
const residue = await Promise.all([
  prisma.page.findUnique({ where: { id: home.id }, select: { manualModifiedAt: true } }),
  prisma.service.findUnique({ where: { id: svc.id }, select: { manualModifiedAt: true } }),
  prisma.project.findUnique({ where: { id: proj.id }, select: { manualModifiedAt: true } }),
  prisma.product.findUnique({ where: { id: prod.id }, select: { manualModifiedAt: true } }),
  prisma.newsPost.findUnique({ where: { id: newsItem.id }, select: { manualModifiedAt: true } }),
]);
const expected = [
  ownershipOrig.page, ownershipOrig.service, ownershipOrig.project, ownershipOrig.product, ownershipOrig.news,
];
const residueOk = residue.every((r, i) => String(r?.manualModifiedAt ?? null) === String(expected[i].manualModifiedAt ?? null));
check('ownership metadata restored', residueOk, residue.map((r) => r?.manualModifiedAt ?? 'null').join(', '));
...
const OUT = outIdx >= 0 ? process.argv[outIdx].split('=')[1] : 'data/redesign/v37/nexttrade/edit-roundtrip.json';
