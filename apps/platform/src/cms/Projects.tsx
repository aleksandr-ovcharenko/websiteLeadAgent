        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              {['Project', 'Location', 'Project status', 'Published', 'Updated', ''].map(col => <th key={col} className="text-left text-[11px] font-semibold text-text-subtle uppercase tracking-wider px-4 py-2 bg-surface-raised whitespace-nowrap">{col}</th>)}
            </tr>
          </thead>
          <tbody>
            {visible.map((project: any) => (
              <tr key={project.id} className="border-b border-border last:border-0 hover:bg-surface-raised/60 transition-colors group">
                <td className="px-4 py-2">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded bg-surface-hover flex-shrink-0 overflow-hidden">
                    ...
                    <button onClick={() => onNavigate('project-editor', project.id)} className="text-[13px] font-medium text-text hover:text-accent transition-colors text-left">{project.title}</button>
                  </div>
                </td>
                <td className="px-4 py-2 text-[12px] text-text-subtle max-w-[180px] truncate">{project.location}</td>
            ...
            <div>
              ...
              <div className="bg-surface border border-border rounded p-4">
                {(() => { const coverM = (media || []).find((m: any) => m.id === coverImageId); return coverM ? (
                  <div className="mb-3">
                    <img src={mediaUrlOf(siteId, coverM)} alt={coverM.alt || title} className="w-full max-h-[220px] object-cover rounded border border-border" />
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <button onClick={() => { setCoverImageId(''); markDirty() }} className="text-[12px] text-danger hover:underline">Remove</button>
                    </div>
                  </div>
                ) : null })()}
                <label className="h-[120px] border border-dashed border-border rounded flex flex-col items-center justify-center gap-2 text-text-subtle hover:bg-surface-raised hover:border-accent hover:text-accent cursor-pointer transition-colors">
                  ...
                  {uploading ? 'Uploading…' : <><IconUpload size={18} /><span className="text-[12px]">{coverImageId ? 'Replace cover image' : 'Click to upload cover'}</span><span className="text-[11px] text-text-subtle">JPG, PNG — recommended 1600×900</span></>}
                </label>
              </div>
            </div>

            <div>
              <p className="text-[11px] font-semibold text-text-subtle uppercase tracking-wider mb-2">Gallery</p>
              <div className="bg-surface border border-border rounded p-4">
                <div className="grid grid-cols-4 gap-2 mb-3">
                  {galleryImageIds.map((id, i) => (
                    <div key={id} className="relative aspect-[4/3] rounded overflow-hidden bg-surface-hover group/img">
                      <img src={mediaUrlOf(siteId, (media || []).find((m: any) => m.id === id))} alt={`${title} ${i + 1}`} className="w-full h-full object-cover" />
                      <button onClick={() => { setGalleryImageIds(imgs => imgs.filter((_, j) => j !== i)); markDirty() }} className="absolute top-1 right-1 w-5 h-5 rounded-full bg-overlay/60 text-text-inverse flex items-center justify-center"><IconX size={9} /></button>
                    </div>
                  ))}
                  <label className="aspect-[4/3] rounded border border-dashed border-border flex flex-col items-center justify-center gap-1.5 text-text-subtle hover:border-accent hover:text-accent hover:bg-success-subtle/20 transition-colors cursor-pointer">
