            ...
            <div>
              ...
              <div className="bg-surface border border-border rounded p-4">
                {(() => { const m = (media || []).find((x: any) => x.id === imageId); return m ? (
                  <div className="mb-3">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <button onClick={() => { setImageId(''); markDirty() }} className="text-[12px] text-danger hover:underline">Remove</button>
                    </div>
                  </div>
                ) : null })()}
                <label className="h-[120px] border border-dashed border-border rounded flex flex-col items-center justify-center gap-2 text-text-subtle hover:bg-surface-raised hover:border-accent hover:text-accent cursor-pointer transition-colors">
                  ...
                  {uploading ? 'Uploading…' : <><IconUpload size={18} /><span className="text-[12px]">{imageId ? 'Replace image' : 'Click to upload or drag image here'}</span><span className="text-[11px] text-text-subtle">JPG, PNG — recommended 800×600</span></>}
                </label>
              </div>
            </div>
