import { constructionModern } from './construction-modern/index.js';
import { constructionFigma } from './construction-figma/index.js';
import { constructionModernV1 } from './construction-modern-v1/index.js';
import { constructionIndustrialV1 } from './construction-industrial-v1/index.js';
import { editorialArchitectureV1 } from './editorial-architecture-v1/index.js';

export const templates: Record<string, typeof constructionModern> = {
  'construction-modern': constructionModern,
  'construction-figma': constructionFigma,
  'construction-modern-v1': constructionModernV1,
  'construction-industrial-v1': constructionIndustrialV1,
  'editorial-architecture-v1': editorialArchitectureV1
};

export { constructionModern } from './construction-modern/index.js';
export { constructionFigma } from './construction-figma/index.js';
export { constructionModernV1 } from './construction-modern-v1/index.js';
export { constructionIndustrialV1 } from './construction-industrial-v1/index.js';
export { editorialArchitectureV1 } from './editorial-architecture-v1/index.js';
export { editorialArchitectureV1Manifest } from './editorial-architecture-v1/manifest.js';
export { resolveHomepageSections, selectBlockItems } from './resolveHomepage.js';
export type { ResolveHomepageInput, ResolveHomepageResult, ResolvedSection } from './resolveHomepage.js';
export { mediaUrlOf } from './media.js';
export type { MediaLike } from './media.js';
export { buildNavItems, navForArea, resolveMenuHref, headerNavTree, flattenNavLeaves } from './nav.js';
export type { ResolvedNavItem } from './nav.js';
export { resolveRouteView } from './routeView.js';
export type { RouteView } from './routeView.js';
export { resolveSectionLimit, applySectionLimit } from './sectionLimits.js';
export type { SectionLike } from './sectionLimits.js';
export { resolveRoute, collectionRoute, entityRoute, entityPreviewPath, collectionKindOf, paginate, clampPageSize, COLLECTION_SLUG_HINTS, DEFAULT_PAGE_SIZE } from './routes.js';
export type { ResolvedRoute, RouteKind, CollectionKind, PageSlice, PagerLink } from './routes.js';
