...
export function chromiumLaunchArgs(explicitNoSandbox) {
    ...
    if (isProduction && noSandboxEnv) {
        throw new Error('PLAYWRIGHT_CHROMIUM_NO_SANDBOX=true is not allowed in production. '
...
export async function launchSandboxedBrowser(launchOptions = {}) {
    const isProduction = process.env.NODE_ENV === 'production';
    ...
    try {
        ...
        if (isProduction) {
            throw new Error(`Chromium sandbox launch failed in production: ${message}. ` +
