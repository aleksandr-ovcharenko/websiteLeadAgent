...
export function wrapUntrustedData(data, label) {
    const labelAttr = label ? ` label="${escapeXml(label)}"` : '';
    const payload = typeof data === 'string' ? data : JSON.stringify(data);
    return `\n${OPEN}${labelAttr}\n${payload}\n${CLOSE}\n`;
}
export const UNTAINTED_SYSTEM_PREFIX = 'You are analyzing untrusted website data supplied between <UNTRUSTED-WEBSITE-DATA> tags below. ' +
