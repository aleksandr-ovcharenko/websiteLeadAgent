...
function isLoopbackIPv4(octs: number[]): boolean {
  return octs[0] === 127;
}

function isLinkLocalIPv4(octs: number[]): boolean {
  return octs[0] === 169 && octs[1] === 254;
}

function isPrivateIPv4(octs: number[]): boolean {
  const [a, b] = octs;
  if (a === 10) return true;
  if (a === 172 && b >= 16 && b <= 31) return true;
  if (a === 192 && b === 168) return true;
  if (a === 100 && b >= 64 && b <= 127) return true; // CGNAT
  if (a === 192 && b === 0 && octs[2] === 0) return true; // IETF
  if (a === 192 && b === 0 && octs[2] === 2) return true; // TEST-NET-1
  if (a === 198 && (b === 18 || b === 19)) return true; // TEST-NET-2/3
  if (a === 198 && b === 51 && octs[2] === 100) return true;
  if (a === 203 && b === 0 && octs[2] === 113) return true;
  if (a === 0) return true;
  if (a >= 224) return true; // multicast/experimental/reserved
  return false;
}

function isCloudMetadataIPv4(octs: number[]): boolean {
  return octs[0] === 169 && octs[1] === 254;
}

function checkIPv4(ip: string): PolicyResult {
  ...
  if (isLoopbackIPv4(octs)) return { allowed: false, reason: 'loopback_ipv4' };
  if (isCloudMetadataIPv4(octs) || isLinkLocalIPv4(octs)) return { allowed: false, reason: 'link_local_ipv4' };
  if (isPrivateIPv4(octs)) return { allowed: false, reason: 'private_ipv4' };
  return { allowed: true };
}
...
function checkIPv6(ip: string): PolicyResult {
  const groups = expandIPv6(ip);
  if (!groups) return { allowed: false, reason: 'invalid_ipv6' };
  ...
  if ((first & 0xffc0) === 0xfe80) return { allowed: false, reason: 'link_local_ipv6' };

  // Unique local fc00::/7
  if ((first & 0xfe00) === 0xfc00) return { allowed: false, reason: 'unique_local_ipv6' };
...
export async function isAllowedUrl(urlString: string): Promise<PolicyResult> {
  let url: URL;
  try {
  ...
  try {
    ...
    for (const addr of addresses) {
      ...
      if (type === 4) {
        ...
        if (!r.allowed) return { allowed: false, reason: `resolved_to_private_ipv4: ${addr}` };
      } else if (type === 6) {
        const r = checkIPv6(addr);
        if (!r.allowed) return { allowed: false, reason: `resolved_to_private_ipv6: ${addr}` };
      }
    }
  } catch (err) {
...
export function isAllowedUrlSync(urlString: string, resolvedAddresses: string[]): PolicyResult {
  let url: URL;
  try {
    url = new URL(urlString);
  } catch {
  ...
  const hostname = url.hostname;
  const ipType = isIP(hostname);
  if (ipType === 4) return checkIPv4(hostname);
  if (ipType === 6) return checkIPv6(hostname);
  if (isBlockedHost(hostname)) return { allowed: false, reason: 'blocked_hostname' };

  if (!resolvedAddresses.length) return { allowed: true };
