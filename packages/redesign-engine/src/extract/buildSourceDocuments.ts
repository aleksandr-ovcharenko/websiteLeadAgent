...
import type { CrawlResult, CrawledImage, CrawledPage, NavigationNode, SourceDocument, SourceDocumentChrome, SourceDocumentCollection, SourceDocumentEvidence, SourceDocumentImage, SourceDocumentLink, SourceDocumentSection } from '../types.js';
...
function extractImageAttributes($: any, img: any, baseUrl: string, pageUrl: string, crawledImage?: CrawledImage): SourceDocumentImage {
  const $img = $(img);
...
function collectSections($: any, root: any, baseUrl: string, pageUrl: string, imageMap: Map<string, SourceDocumentImage>, region: SourceDocumentSection['region']): SourceDocumentSection[] {
  ...
  function startSection(headingEl?: any) {
    flush();
...
function buildSourceDocument(crawledPage: CrawledPage, index: number, baseUrl: string): SourceDocument {
  ...
  const collections: SourceDocumentCollection[] = detectCollections($, baseUrl, pageUrl, imageMap);
