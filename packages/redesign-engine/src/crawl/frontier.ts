import type { CrawlCandidateSource, CrawlPlanEntry } from '../types.js';
...
export const SOURCE_PRIORITY: Record<CrawlCandidateSource, number> = {
...
export function isLowValueUrl(url: string): boolean {
  try {
    const u = new URL(url);
    if (LOW_VALUE_RE.test(u.pathname)) return true;
    // Path pagination beyond page 1 (/page/2/, /stranica/3/) is low value.
    if (/\/(page|stranica|str|p)\/\d{2,}\/?$/i.test(u.pathname)) return true;
    if (u.searchParams.has('sort') || u.searchParams.has('order') || u.searchParams.has('filter')) return true;
    const page = u.searchParams.get('page') || u.searchParams.get('p');
    if (page && Number(page) > 2) return true;
    return false;
  } catch {
    return false;
  }
...
export class CrawlFrontier {
  ...
  private itemByKey = new Map<string, FrontierItem>();
  private seenKeys = new Set<string>();
  private order = 0;
  private ancestorDescendants = new Map<string, number>();
  readonly plan = new Map<string, CrawlPlanEntry>();

  constructor(private maxDepth: number) {}

  private effectivePriority(url: string, depth: number, source: CrawlCandidateSource, extra = 0): number {
    let priority = SOURCE_PRIORITY[source] + depth * 10 + extra;
    if (isLowValueUrl(url)) priority += FRONTIER_PRIORITY.LOW_VALUE;
    return priority;
  }

  /** Returns true when the URL was newly enqueued. */
  add(url: string, depth: number, source: CrawlCandidateSource, extraPriority = 0): boolean {
    ...
    if (this.seenKeys.has(key)) {
      ...
      if (existing && priority < existing.priority) {
        ...
        if (e) { e.priority = priority; e.source = source; e.depth = existing.depth; }
      }
      return false;
    }
   ...
   */
  private applyHubBoost(url: string): void {
    let pathname: string;
  ...
  next(): FrontierItem | undefined {
    ...
    for (let i = 1; i < this.items.length; i++) {
      ...
      if (it.priority < best.priority || (it.priority === best.priority && it.order < best.order)) bestIdx = i;
    }
  ...
  mark(entry: FrontierItem | string, patch: Partial<CrawlPlanEntry>): void {
  ...
  finalizePlan(result: 'BUDGET_EXHAUSTED' | 'NOT_ATTEMPTED'): CrawlPlanEntry[] {
    for (const p of this.plan.values()) {
      if (!p.attempted && !p.result) p.result = result;
    }
