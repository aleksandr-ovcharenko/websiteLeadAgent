...
function bestDisplayName(graph: SourceContentGraph, docs: SourceDocument[]): { name?: string; src: string } {
  ...
  for (const c of cand) {
    const v = (c.v || '').trim();
    if (v && v.length >= 2 && v.length <= 60 && !isSeoTitle(v)) return { name: v, src: c.src };
  }
...
export function buildSiteContentPlanV2(opts: {
  ...
  const docImage = (d?: SourceDocument) => d?.images?.find((i) => !isPlaceholderMedia(i.src))?.src;
  ...
  const pushEntity = (type: EntityType, e: { id: string; title: string; sourceDocumentIds: string[]; imageIds?: string[]; description?: string; evidence?: any[] }, extra?: Partial<PlannedEntity>) => {
    ...
    const media = (e.imageIds || []).map((id) => graph.media.find((m) => m.id === id)?.src).filter(Boolean) as string[];
    const img = media.find((s) => !isPlaceholderMedia(s)) || docImage(detailDoc) || docImage(docIds.map((id) => docById.get(id)).find(Boolean) as any);
    entities.push({
