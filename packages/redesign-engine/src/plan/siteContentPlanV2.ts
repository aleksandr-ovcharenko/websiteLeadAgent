...
export interface PlannedDynamicSection {
  ...
  /** Optional grounded CTA — e.g. "Открыть конструктор" → source configurator page. */
...
export function buildSiteContentPlanV2(opts: {
  ...
  // grounded entity data. Nothing is invented.
  {
    ...
    if (cfgDoc && clause && !dynamicSections.some((d) => d.kind === 'PROCESS')) {
      ...
      if (prodCount) items.push({ title: 'Выберите модель', text: `${prodCount} моделей в каталоге` });
      ...
      if (kits.length) items.push({ title: 'Выберите комплектацию', text: kits.join(' · ') });
      if (items.length >= 2) {
  ...
  const lateDyn: typeof plannedSections = [];
  for (const d of dynamicSections) {
    if (['PROCESS', 'ADVANTAGES', 'REVIEWS', 'FAQ', 'TEAM', 'PRICING', 'PARTNERS', 'STATS'].includes(d.kind)) {
      const sec = { type: 'dynamic' as const, heading: d.heading || d.kind, origin: 'SOURCE_CONTENT' as const, entityIds: [], dynamicSectionId: d.id, rationale: `${d.kind} section preserved from source` };
  ...
  const media = {
    ...
    hero: (() => {
      ...
      const freeImg = (m: any) => { const u = norm(m?.src); return u && !entitySrcs.has(u) && !isPlaceholderMedia(u) && !/\.svg(\?|$)|logo|icon|sprite|removebg/i.test(u) ? u : undefined; };
  ...
  const hostTok = (() => { try { return new URL(opts.baseUrl).hostname.split('.')[0]; } catch { return ''; } })();
