...
async function loadPackages(repoRoot: string): Promise<LockPackage[]> {
  ...
  for (const [nodePath, pkg] of Object.entries(pkgs)) {
    const p = pkg as any;
    if (!nodePath.includes('node_modules/') || !p?.version) continue;
...
function contentHash(packages: LockPackage[]): string {
...
export async function linkSiteBuildSnapshot(
  prisma: PrismaClient,
  siteBuildId: string,
  snapshotId: string
): Promise<void> {
