...
export interface FactCandidate {
  rawValue: string;
  attemptedType: AttemptedFactType;
  source: string;           // e.g. 'chrome-phone', 'section-phone', 'jsonld'
  context?: string;         // nearby/label text used for context checks
  sourceDocumentId?: string;
}

export interface RejectedFactCandidate extends FactCandidate {
...
export function validatePhoneCandidate(raw: string, context = ''): FactValidationResult {
  const value = raw.replace(/\s+/g, ' ').trim();
  if (!value) return { ok: false, rejectionReason: 'EMPTY' };
  if (/[a-zA-Zа-яА-Я@]/.test(value)) return { ok: false, rejectionReason: 'CONTAINS_NON_PHONE_CHARS' };
  if (YEAR_RANGE_RE.test(value)) return { ok: false, rejectionReason: 'YEAR_RANGE' };
  if (BANK_CTX_RE.test(context) || BANK_CTX_RE.test(value)) return { ok: false, rejectionReason: 'BANK_ACCOUNT' };

  // Concatenated unrelated numbers: more than one phone-length digit run.
  const digitRuns = value.match(/\d[\d\s()\-]{5,}\d/g) || [];
  if (digitRuns.length > 1) return { ok: false, rejectionReason: 'CONCATENATED' };

  const digits = value.replace(/\D/g, '');
  if (digits.length < 7) return { ok: false, rejectionReason: 'TOO_FEW_DIGITS' };
  if (digits.length > 15) return { ok: false, rejectionReason: 'TOO_MANY_DIGITS' };
  ...
  if (/^(18|19|20)\d{2}$/.test(digits)) return { ok: false, rejectionReason: 'YEAR' };
...
export function validateEmailCandidate(raw: string, _context = ''): FactValidationResult {
  const match = raw.match(EMAIL_EXTRACT_RE);
  if (!match) return { ok: false, rejectionReason: 'NOT_AN_EMAIL' };
  let email = match[0];

  if (IMAGE_OR_ASSET_RE.test(email)) return { ok: false, rejectionReason: 'IMAGE_FILENAME' };
  ...
  if (/^[\d.\-_]+$/.test(local)) return { ok: false, rejectionReason: 'NUMERIC_LOCAL' };

  const noisy = local.match(/^[\d][\d.\-_]{3,}(?=[A-Za-z])/);
  if (noisy) {
    local = local.slice(noisy[0].length);
    if (local.length < 2) return { ok: false, rejectionReason: 'NOISY_PREFIX' };
    email = `${local}@${domain}`;
  }

  if (PLACEHOLDER_DOMAIN_RE.test(domain)) return { ok: false, rejectionReason: 'PLACEHOLDER' };
  if (!STRICT_EMAIL_RE.test(email)) return { ok: false, rejectionReason: 'INVALID_SYNTAX' };
...
export function validateAddressCandidate(raw: string, context = '', source = ''): FactValidationResult {
  ...
  for (let i = value.length - 1; i > 0; i--) {
    ...
    if (STREET_TOKEN_RE.test(tail) && /\d/.test(tail)) {
      ...
      for (let j = tail.indexOf('. '); j !== -1; j = tail.indexOf('. ', j + 2)) {
        if (!ABBR_DOT.test(tail.slice(0, j).trimEnd().split(/\s+/).pop() || '')) { sentEnd = j; break; }
      }
  ...
  if (value.length < 10) return { ok: false, rejectionReason: 'TOO_SHORT' };
  if (value.length > 160) return { ok: false, rejectionReason: 'TOO_LONG' };
  if (/@/.test(value)) return { ok: false, rejectionReason: 'CONTAINS_EMAIL' };
  ...
  if (BANK_CTX_RE.test(value)) return { ok: false, rejectionReason: 'BANK_ACCOUNT' };
  if (/\d[\d\s()\-]{6,}\d/.test(value)) {
    return { ok: false, rejectionReason: 'CONTAINS_PHONE' };
  }
  if (CATALOG_OR_SERVICE_TEXT_RE.test(value)) return { ok: false, rejectionReason: 'CATALOG_OR_SERVICE_TEXT' };

  // Structured / labelled evidence wins outright.
  if (/jsonld|postaladdress|schema/i.test(source)) return { ok: true, value };
  if (ADDRESS_LABEL_RE.test(context) || ADDRESS_LABEL_RE.test(value)) return { ok: true, value };

  // Otherwise require a street-type token AND a house/number component.
  if (STREET_TOKEN_RE.test(value) && /\d/.test(value)) return { ok: true, value };
...
export function validateEmployeeCount(raw: string, _context = ''): FactValidationResult {
  const value = raw.replace(/\s+/g, ' ').trim();
  if (!EMPLOYEE_WORD_RE.test(value)) return { ok: false, rejectionReason: 'NO_EMPLOYEE_CONTEXT' };
  const numMatch = value.match(/\d[\d\s]*/);
  if (!numMatch) return { ok: false, rejectionReason: 'NO_NUMBER' };
  const num = Number(numMatch[0].replace(/\s/g, ''));
  if (!Number.isFinite(num)) return { ok: false, rejectionReason: 'NO_NUMBER' };
  // Animation/counter defaults ("0 сотрудников") are not a business fact.
  if (num === 0) return { ok: false, rejectionReason: 'COUNTER_ZERO' };
  if (num > 10_000_000) return { ok: false, rejectionReason: 'IMPLAUSIBLE' };
  return { ok: true, value };
}
...
export function validateFoundingDate(raw: string, _context = ''): FactValidationResult {
  const value = raw.trim();
  if (YEAR_RANGE_RE.test(value)) return { ok: false, rejectionReason: 'YEAR_RANGE' };
  const m = value.match(/(?<!\d)(18\d{2}|19\d{2}|20\d{2})(?!\d)/);
  if (!m) return { ok: false, rejectionReason: 'NO_YEAR' };
  const year = Number(m[0]);
  if (year < 1800 || year > new Date().getFullYear() + 1) return { ok: false, rejectionReason: 'IMPLAUSIBLE_YEAR' };
...
export function validateRegistrationId(raw: string, context = ''): FactValidationResult {
  const digits = raw.replace(/\D/g, '');
  if (digits.length !== 9) return { ok: false, rejectionReason: 'NOT_NINE_DIGITS' };
  if (!REGISTRATION_CTX_RE.test(context)) return { ok: false, rejectionReason: 'REGISTRATION_WITHOUT_LABEL' };
...
export function validateFactCandidate(c: FactCandidate): FactValidationResult {
...
export class FactGate {
  ...
  accept(c: FactCandidate): string | undefined {
