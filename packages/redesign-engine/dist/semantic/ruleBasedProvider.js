...
function hasRe(re, ...texts) {
    for (const t of texts)
        if (t && re.test(t))
            return true;
    return false;
}
function countRe(re, ...texts) {
    let n = 0;
    for (const t of texts)
        if (t)
            n += (t.match(re) || []).length;
...
function isGenericCompanyName(name) {
    ...
    if (generic.test(n) || n.startsWith('site title'))
        return true;
    // reject values that look like domain names used as site names
    if (/(?:www\.)?[a-z0-9-]+\.[a-z]{2,}(?:\/[a-z0-9-]*)?$/i.test(name.trim()))
        return true;
    return false;
}
...
function isGroupLabel(title) {
    if (isStatusTitle(title))
        return 'status';
...
function titleFromDescription(description) {
    ...
    if (text.length < 3)
        return undefined;
    if (OBJECT_REF_RE.test(text) || PROJECTS_RE.test(text))
...
function isConcreteProjectEvidence(item, resolvedUrl, title) {
    ...
    if (item.description && (OBJECT_REF_RE.test(item.description) || PROJECTS_RE.test(item.description)))
        return true;
    if (item.image && title && /[\d№«""“”]/u.test(title) && OBJECT_REF_RE.test(title))
        return true;
    return false;
}
...
function isInvestorOrReportContent(text, url) {
    const t = norm(text || '');
...
export function pageCategoryAndSubType(type, doc) {
    if (type === 'HOME')
        return { category: 'HOME' };
    const contentTypes = ['SERVICES_INDEX', 'SERVICE_DETAIL', 'PROJECTS_INDEX', 'PROJECT_DETAIL', 'NEWS_INDEX', 'NEWS_DETAIL', 'VACANCIES_INDEX', 'VACANCY_DETAIL', 'PRODUCTS_INDEX', 'PRODUCT_DETAIL'];
    ...
    if (type === 'CONTACTS')
        return { category: 'UTILITY', subType: 'CONTACTS' };
    if (type === 'LEGAL')
        return { category: 'UTILITY', subType: 'LEGAL' };
    if (type === 'OTHER')
        return { category: 'UTILITY', subType: 'OTHER' };
    ...
    if (INVESTOR_RE.test(allText) || /\b(investor|shareholder|акционер|инвестор|investor relations)\b/iu.test(allText))
        return { category: 'CORPORATE', subType: 'INVESTOR_RELATIONS' };
    if (COMPLIANCE_RE.test(allText))
        return { category: 'CORPORATE', subType: 'COMPLIANCE' };
    if (REPORT_DOCUMENT_RE.test(allText))
        return { category: 'CORPORATE', subType: 'DOCUMENTS' };
    if (CERTIFICATES_RE.test(allText))
        return { category: 'CORPORATE', subType: 'CERTIFICATES' };
    if (MANAGEMENT_RE.test(allText))
        return { category: 'CORPORATE', subType: 'MANAGEMENT' };
    if (TEAM_RE.test(allText))
        return { category: 'CORPORATE', subType: 'TEAM' };
    if (HISTORY_RE.test(allText))
        return { category: 'CORPORATE', subType: 'HISTORY' };
    if (MISSION_RE.test(allText))
        return { category: 'CORPORATE', subType: 'MISSION' };
...
function isCategoryLandingUrl(url, allUrls) {
    if (!url)
        return false;
...
function isHomePage(doc, baseUrl) {
    if (doc.isHomepage)
        return true;
...
function structuredDataTypes(doc) {
    const types = [];
    for (const sd of doc.structuredData || []) {
        if (sd['@type'])
            types.push(String(sd['@type']));
        if (Array.isArray(sd['@graph'])) {
            for (const g of sd['@graph'])
                if (g['@type'])
                    types.push(String(g['@type']));
        }
    }
    return types.map((t) => t.toLowerCase());
}
function breadcrumbLabels(doc) {
    return (doc.chrome.nav?.breadcrumbs || []).map((b) => b.label).join(' ');
}
function allNavLabels(doc) {
    const walk = (nodes) => nodes.flatMap((n) => [n.label, ...(n.children ? walk(n.children) : [])]);
    const primary = doc.chrome.nav?.primary || [];
    const secondary = doc.chrome.nav?.secondary || [];
    return [...walk(primary), ...walk(secondary)].join(' ');
}
function parentSectionHeading(doc, collectionId) {
...
function parentSectionRegion(doc, collectionId) {
    for (const sec of doc.sections || []) {
        if ((sec.collections || []).some((c) => c.id === collectionId)) {
...
function navAncestryForUrl(url, doc) {
...
function evidence(type, value, confidence, extra = {}) {
    return { type, value, confidence, ...extra };
}
function pageEvidence(doc, type, value, confidence) {
    return { type, value, confidence, sourceDocumentId: doc.id };
}
export class RuleBasedSemanticProvider {
    name = 'rule-based';
    model = 'none';
    promptVersion = '0.1';
    temperature = 0;
    ...
    classifyPage(ctx) {
        ...
        const add = (type, baseScore, reasons) => signals.push({ type, score: baseScore, evidence: reasons });
        const detailType = {
            ABOUT: 'ABOUT',
        ...
        const pathMatch = (re, score, type) => {
            if (segments.some((s) => re.test(s)) || re.test(pathText)) {
                ...
                const resolved = (detail ? detailType[type] : type);
                let sc = score;
                if (detail)
                    sc += 10;
                if (re.test(h1) || re.test(title))
                    sc += 8;
        ...
        if (isHomePage(doc, baseUrl))
            add('HOME', 20, [pageEvidence(doc, 'url', doc.url, 0.95)]);
        // Title / H1 / meta content classification
        // Use only page-local content (title, h1, meta, breadcrumb). Global nav labels are too noisy.
        if (hasRe(ABOUT_RE, title, h1, meta, bc))
            add('ABOUT', 6, [pageEvidence(doc, 'heading', h1 || title, 0.7)]);
        if (hasRe(SERVICES_RE, title, h1, meta, bc))
            add('SERVICES_INDEX', 7, [pageEvidence(doc, 'heading', h1 || title, 0.75)]);
        if (hasRe(PROJECTS_RE, title, h1, meta, bc))
            add('PROJECTS_INDEX', 7, [pageEvidence(doc, 'heading', h1 || title, 0.75)]);
        if (hasRe(NEWS_RE, title, h1, meta, bc))
            add('NEWS_INDEX', 8, [pageEvidence(doc, 'heading', h1 || title, 0.8)]);
        if (hasRe(VACANCIES_RE, title, h1, meta, bc))
            add('VACANCIES_INDEX', 7, [pageEvidence(doc, 'heading', h1 || title, 0.75)]);
        if (hasRe(PRODUCTS_RE, title, h1, meta, bc))
            add('PRODUCTS_INDEX', 7, [pageEvidence(doc, 'heading', h1 || title, 0.75)]);
        if (hasRe(CONTACTS_RE, title, h1, meta, bc))
            add('CONTACTS', 9, [pageEvidence(doc, 'heading', h1 || title, 0.85)]);
        if (hasRe(LEGAL_RE, title, h1, meta, bc))
            add('LEGAL', 10, [pageEvidence(doc, 'heading', h1 || title, 0.85)]);
        // Structured data
        if (structured.includes('contactpage') || structured.includes('contact'))
            add('CONTACTS', 12, [pageEvidence(doc, 'jsonld', structured.join(', '), 0.9)]);
        if (structured.includes('jobposting'))
            add('VACANCIES_INDEX', 12, [pageEvidence(doc, 'jsonld', 'JobPosting', 0.9)]);
        if (structured.includes('newsarticle'))
            add('NEWS_DETAIL', 11, [pageEvidence(doc, 'jsonld', 'NewsArticle', 0.9)]);
        if (structured.includes('article'))
            add('NEWS_DETAIL', 9, [pageEvidence(doc, 'jsonld', 'Article', 0.85)]);
        if (structured.includes('product'))
            add('PRODUCT_DETAIL', 11, [pageEvidence(doc, 'jsonld', 'Product', 0.9)]);
        if (structured.includes('service'))
            add('SERVICE_DETAIL', 11, [pageEvidence(doc, 'jsonld', 'Service', 0.9)]);
        ...
        if (ancestry.length) {
            ...
            if (SERVICES_RE.test(ancestorText))
                add('SERVICE_DETAIL', 8, [pageEvidence(doc, 'nav-ancestor', ancestorText, 0.7)]);
            if (PROJECTS_RE.test(ancestorText))
                add('PROJECT_DETAIL', 8, [pageEvidence(doc, 'nav-ancestor', ancestorText, 0.7)]);
            if (NEWS_RE.test(ancestorText))
                add('NEWS_DETAIL', 8, [pageEvidence(doc, 'nav-ancestor', ancestorText, 0.7)]);
            if (VACANCIES_RE.test(ancestorText))
                add('VACANCY_DETAIL', 8, [pageEvidence(doc, 'nav-ancestor', ancestorText, 0.7)]);
        }
    ...
    classifyCollection(ctx) {
        ...
        const urls = items.map((i) => i.url).filter((u) => Boolean(u));
        ...
        const allUrlsInternal = urls.length > 0 && urls.every((u) => {
            try {
                return new URL(u).hostname === new URL(baseUrl).hostname;
            }
            catch {
                return true;
            }
        ...
        if (items.length >= 2 && !allUrlsRoot) {
            const goodUrls = urls.filter((u) => { try {
            ...
            const matches = (re, text) => (text.match(re) || []).length;
            const itemMatches = (re) => items.filter((i) => re.test(`${norm(i.title)} ${norm(i.description || '')} ${i.url || ''}`)).length;
            const pageType = page.type;
            const pageBoost = (index, detail) => (pageType === index || pageType === detail ? 3 : 0);
            // Type candidate is a weak hint, not a license; strong textual evidence must back it up
            const candidateBoost = (cand) => (typeCandidate === cand ? 2 : 0);
            const imageBonus = items.some((i) => i.image) ? 2 : 0;
            // Generic scores: section heading > page text > descriptions > selector > item titles > urls
            const scoreFor = (projectRe, secWeight = 8, pageWeight = 3, descWeight = 2, selWeight = 2, itemWeight = 1, urlWeight = 1) => {
                let s = 0;
            ...
            const best = [
                { subtype: 'SERVICES', score: serviceScore },
                { subtype: 'PROJECTS', score: projectScore },
                { subtype: 'NEWS', score: newsScore },
                { subtype: 'VACANCIES', score: vacancyScore },
                { subtype: 'PRODUCTS', score: productScore },
            ...
            const headingSupports = (subtype) => {
                if (subtype === 'PROJECTS')
                    return PROJECTS_RE.test(headingText) || PORTFOLIO_RE.test(headingText);
                if (subtype === 'PRODUCTS')
                    return PRODUCTS_RE.test(headingText) || PRODUCT_CATALOG_RE.test(headingText);
                if (subtype === 'SERVICES')
                    return SERVICES_RE.test(headingText);
                if (subtype === 'NEWS')
                    return NEWS_RE.test(headingText);
                if (subtype === 'VACANCIES')
                    return VACANCIES_RE.test(headingText);
                return false;
            };
            ...
            const hasCollectionEvidence = (subtype) => {
                if (headingSupports(subtype))
                    return true;
                if (subtype === 'PROJECTS') {
            ...
            const pageSupports = (subtype) => {
                // Detail pages require a heading to avoid classifying the whole page wrapper
                if (subtype === 'PROJECTS')
                    return pageType === 'PROJECTS_INDEX' || pageType === 'HOME';
                if (subtype === 'PRODUCTS')
                    return pageType === 'PRODUCTS_INDEX' || pageType === 'PROJECTS_INDEX' || pageType === 'HOME';
                if (subtype === 'SERVICES')
                    return pageType === 'SERVICES_INDEX';
                if (subtype === 'NEWS')
                    return pageType === 'NEWS_INDEX';
                if (subtype === 'VACANCIES')
                    return pageType === 'VACANCIES_INDEX';
                return false;
            };
            ...
            const candidates = [best, ...[serviceScore, projectScore, newsScore, vacancyScore, productScore]
                    .map((score, idx) => ({
                    subtype: ['SERVICES', 'PROJECTS', 'NEWS', 'VACANCIES', 'PRODUCTS'][idx],
                    score,
                }))
                    .filter((c) => c.score >= best.score * 0.7)];
            for (const candidate of candidates.sort((a, b) => b.score - a.score)) {
                if (candidate.score < minScore || !(hasImages || hasUrls || hasDescriptions))
                    continue;
                ...
                if (!hasCollectionEvidence(candidate.subtype))
                    continue;
    ...
    classifySection(ctx) {
        ...
        const evidenceList = [pageEvidence(doc, 'section', heading, 0.6)];
        ...
        for (const col of section.collections || []) {
            ...
            if (cc) {
                if (cc.type === 'LANGUAGE_SWITCHER')
                    return { sectionId: section.id, type: 'LANGUAGE_SWITCHER', confidence: cc.confidence, evidence: evidenceList };
                if (cc.type === 'THEME_WIDGET')
                    return { sectionId: section.id, type: 'THEME_WIDGET', confidence: cc.confidence, evidence: evidenceList };
                if (cc.type === 'ADVERTISEMENT')
                    return { sectionId: section.id, type: 'ADVERTISEMENT', confidence: cc.confidence, evidence: evidenceList };
                if (cc.type === 'NAVIGATION')
                    return { sectionId: section.id, type: 'NAVIGATION', confidence: cc.confidence, evidence: evidenceList };
            }
        }
        ...
        if (contactSignals.test(heading + ' ' + text) || page.type === 'CONTACTS') {
            ...
            if (hasContact)
                return { sectionId: section.id, type: 'CONTACT_DETAILS', confidence: 0.85, evidence: evidenceList };
        }
        // Hero
        ...
        if (page.type === 'SERVICE_DETAIL' || page.type === 'SERVICES_INDEX')
            return { sectionId: section.id, type: 'SERVICE_DESCRIPTION', confidence: 0.7, evidence: evidenceList };
        if (page.type === 'PROJECT_DETAIL' || page.type === 'PROJECTS_INDEX')
            return { sectionId: section.id, type: 'PROJECT_DESCRIPTION', confidence: 0.7, evidence: evidenceList };
        if (page.type === 'NEWS_DETAIL' || page.type === 'NEWS_INDEX')
            return { sectionId: section.id, type: 'ARTICLE_BODY', confidence: 0.75, evidence: evidenceList };
    ...
    extractCompanyDescription(ctx, home) {
        ...
        if (firstMain)
            return firstMain.paragraphs.join(' ').slice(0, 500);
        return undefined;
    }
    findFact(ctx, type) {
    ...
    extractContacts(ctx) {
        ...
        if (!phones.length && !emails.length && !addresses.length && !socialLinks.length)
            return undefined;
        return {
            id: id(),
    ...
    extractServices(ctx) {
        ...
        for (const doc of ctx.sourceDocuments) {
            ...
            for (const col of doc.collections || []) {
                ...
                if (cc?.type !== 'CONTENT_COLLECTION' || cc.contentSubtype !== 'SERVICES')
                    continue;
                for (const item of col.items) {
                    ...
                    if (resolvedUrl) {
                        try {
                            if (new URL(resolvedUrl).pathname === new URL(doc.url).pathname)
                                continue;
                        }
                        catch { }
                    }
                    if (resolvedUrl && byUrl.has(resolvedUrl)) {
                        ...
                        if (item.description && !existing.description)
                            existing.description = item.description.slice(0, 1200);
                    ...
                    if (isGenericHeading(item.title) && !item.description)
                        continue;
                    if (!resolvedUrl && !item.description)
                        continue;
    ...
    extractProjects(ctx) {
        ...
        for (const doc of ctx.sourceDocuments) {
            ...
            for (const col of doc.collections || []) {
                const cc = colls.find((c) => c.collectionId === col.id);
                if (cc?.type !== 'CONTENT_COLLECTION' || cc.contentSubtype !== 'PROJECTS')
                    continue;
                ...
                let currentStatus;
                for (const item of col.items) {
                    let title = item.title && item.title.trim() ? item.title : titleFromDescription(item.description || '');
                    if (!title)
                        continue;
                    // The collection heading itself is not an entity, even if it appears as an item.
                    if (norm(title) === norm(col.heading || ''))
                        continue;
                    ...
                    if (groupKind === 'status' || groupKind === 'category' || categoryLanding || item.isGroup) {
                        if (groupKind === 'status')
                            currentStatus = title;
                        if (groupKind === 'category' || categoryLanding || item.isGroup)
                            currentCategory = title;
                        continue;
                    }
                    // A collection heading like "Completed Projects" or a status label is not a concrete project.
                    if (isGenericHeading(title) && !item.description && !item.image)
                        continue;
                    if (resolvedUrl) {
                        try {
                            if (new URL(resolvedUrl).pathname === new URL(doc.url).pathname)
                        ...
                        if (byUrl.has(resolvedUrl)) {
                            ...
                            if (item.image) {
                                ...
                                if (imgId && !existing.imageIds?.includes(imgId))
                                    existing.imageIds = [...(existing.imageIds || []), imgId];
                            }
                            if (item.description && !existing.description)
                                existing.description = item.description.slice(0, 1200);
                            // Update group metadata if it was missing.
                            if (!existing.category && currentCategory)
                                existing.category = currentCategory;
                            if (!existing.projectStatus && currentStatus)
                                existing.projectStatus = currentStatus;
                            continue;
                        }
                    }
                    // A project card needs at least a meaningful title and either a URL or an image; description is optional
                    if (!resolvedUrl && !item.image && !item.description)
                        continue;
                    // Reject entries that are really timeline facts, CTAs, or awards without a project object.
                    if (!isConcreteProjectEvidence(item, resolvedUrl, title))
                        continue;
                    const imgs = item.image ? [this.imageIdForSrc(ctx, item.image.src)] : [];
    ...
    extractNews(ctx) {
        ...
        for (const doc of ctx.sourceDocuments) {
            const pc = ctx.pageClassifications.get(doc.id);
            if (pc?.type !== 'NEWS_DETAIL')
                continue;
            const title = isGenericHeading(doc.h1 || '') ? lastSegment(doc.url).replace(/[-_]/g, ' ') : cleanCompanyName(doc.h1 || doc.title);
            const pageText = `${doc.h1 || ''} ${doc.title || ''} ${doc.metaDescription || ''}`;
            if (isInvestorOrReportContent(pageText, doc.url))
                continue;
            const date = this.extractDate(ctx, doc);
            const desc = this.descriptionFromMainSections(doc, ctx, 'ARTICLE_BODY');
            const newsEvidence = [evidence('page-detail', title, pc.confidence, { sourceDocumentId: doc.id, sourceUrl: doc.url }), ...pc.evidence];
            if (date) {
        ...
        for (const doc of ctx.sourceDocuments) {
            const pc = ctx.pageClassifications.get(doc.id);
            if (pc?.type !== 'NEWS_INDEX')
                continue;
            const colls = ctx.collectionClassifications.get(doc.id) || [];
            for (const col of doc.collections || []) {
                const cc = colls.find((c) => c.collectionId === col.id);
                if (cc?.type !== 'CONTENT_COLLECTION' || cc.contentSubtype !== 'NEWS')
                    continue;
                for (const item of col.items) {
                    if (!item.title)
                        continue;
                    ...
                    if (!resolvedUrl && !item.description)
                        continue;
                    if (isInvestorOrReportContent(item.title, resolvedUrl || item.url))
                        continue;
    ...
    extractVacancies(ctx) {
        ...
        for (const doc of ctx.sourceDocuments) {
            ...
            for (const col of doc.collections || []) {
                const cc = colls.find((c) => c.collectionId === col.id);
                if (cc?.type !== 'CONTENT_COLLECTION' || cc.contentSubtype !== 'VACANCIES')
                    continue;
                for (const item of col.items) {
                    if (!item.title)
                        continue;
    ...
    extractProducts(ctx) {
        ...
        for (const doc of ctx.sourceDocuments) {
            ...
            for (const col of doc.collections || []) {
                const cc = colls.find((c) => c.collectionId === col.id);
                if (cc?.type !== 'CONTENT_COLLECTION' || cc.contentSubtype !== 'PRODUCTS')
                    continue;
                for (const item of col.items) {
                    if (!item.title)
                        continue;
    ...
    descriptionFromMainSections(doc, ctx, preferredType) {
        const sections = ctx.sectionClassifications.get(doc.id) || [];
        const parts = [];
        for (const sec of doc.sections) {
            const sc = sections.find((s) => s.sectionId === sec.id);
    ...
    resolveUrl(url, baseUrl) {
        ...
        try {
            ...
            return url.toLowerCase().replace(/#$/, '').replace(/\/$/, '') || undefined;
        }
    }
    ...
    extractDate(ctx, doc) {
        ...
        for (const sec of doc.sections) {
            for (const p of sec.paragraphs) {
                ...
                if (m)
                    return { value: m[0], evidence: evidence('visible-date', m[0], 0.5, { sourceDocumentId: doc.id, sourceSectionId: sec.id }) };
            }
        }
        return undefined;
    }
    extractDateFromItem(item, doc) {
        ...
        if (item.description && YEAR_RE.test(item.description)) {
            ...
            if (m)
                return { value: m[0], evidence: evidence('description-year', m[0], 0.4, { sourceDocumentId: doc.id }) };
        }
        return undefined;
    }
}
