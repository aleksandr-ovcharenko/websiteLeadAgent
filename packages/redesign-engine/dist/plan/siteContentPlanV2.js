...
function detectDynamicKind(heading, items, classification) {
    ...
    if (texts.length >= 2 && texts.filter((t) => t.endsWith('?')).length >= Math.ceil(texts.length / 2))
        return { kind: 'FAQ' };
    if (texts.length >= 2 && texts.filter((t) => /\d/.test(t) && (/[₽$€Br%]|\d+\s*\+?\s*(м2|м²|лет|год|проект|домов|объект|опыт)/iu.test(t) || /^\d+\s*\+$/.test(t.trim()))).length >= texts.length / 2)
        return { kind: 'STATS' };
...
export function buildSiteContentPlanV2(opts) {
    ...
    const decodeURIComponentSafe = (u) => { try {
        ...
        for (const c of d?.collections || [])
            for (const i of c.items || []) {
                if (/что входит/i.test(i.title || '')) {
                    const v = (i.text || i.description || '').trim();
                    if (v && v.length <= 60 && !kits.includes(v))
                        kits.push(v);
                }
            }
        if (kits.length)
            out['Комплектация'] = kits.slice(0, 8).join(' · ');
        return out;
    };
    const pushEntity = (type, e, extra) => {
        ...
        const isPlanish = (u) => !u || /plan|план|схем|schema|layout/i.test(decodeURIComponentSafe(u)) || /\.png(\?|$)/i.test(u);
        const renderFirst = (list) => [...list].sort((a, b) => Number(isPlanish(a)) - Number(isPlanish(b)));
        const orderedMedia = renderFirst(media.filter((s) => !isPlaceholderMedia(s)));
        const img = orderedMedia[0] || docImage(docIds.map((id) => docById.get(id)).find(Boolean));
