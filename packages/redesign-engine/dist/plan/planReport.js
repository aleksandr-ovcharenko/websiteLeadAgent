...
export function buildPlanReport(plan, graph, docs) {
    const p = plan;
    const out = [];
    ...
    if (p.siteIdentity.legalName)
        out.push(`- legal name: ${p.siteIdentity.legalName}`);
    if (p.siteIdentity.industry)
        out.push(`- industry: ${p.siteIdentity.industry}`);
    if (p.siteIdentity.unp)
        out.push(`- UNP: ${p.siteIdentity.unp}`);
