const li = (items) => items.length ? items.map((x) => `- ${x}`).join('\n') : '- (none)';
const ent = (e) => `- **${e.title}** — ${e.sourceUrls.join(', ') || '—'} — home:${e.onHomepage ? 'yes' : 'no'}${e.primaryImage ? ` — img:${e.primaryImage.slice(0, 60)}` : ''}${e.summary ? `\n  > ${e.summary.slice(0, 120)}` : ''}`;
export function buildPlanReportV2(plan) {
    const o = [`# SiteContentPlan V2 — ${plan.siteKey}`, '',
    ...
    for (const t of ['service', 'project', 'product', 'article', 'news', 'vacancy']) {
        const list = plan.entities.filter((e) => e.type === t);
        o.push('', `## ${t.toUpperCase()} (${list.length})`, list.length ? list.map(ent).join('\n') : `(none)`);
    }
    o.push('', `## Dynamic sections (${plan.dynamicSections.length})`, li(plan.dynamicSections.map((d) => `${d.kind} — ${d.heading || d.id} — ${d.items.length} items — on ${d.sourcePages.length} page(s)${d.ignoredReason ? ` — IGNORED: ${d.ignoredReason}` : ''}${d.items.length ? ` — e.g. ${d.items.slice(0, 3).map((i) => i.title || i.text).filter(Boolean).join('; ')}` : ''}`)), '', '## Media', `- logo: ${plan.media.logo || '—'}`, `- hero: ${plan.media.hero || '—'}`, `- images: ${plan.media.images.length}`, '', '## Omitted content', li(plan.omittedContent.map((x) => `${x.what} — ${x.reason}`)), '', '## Warnings', li(plan.warnings));
    return o.join('\n') + '\n';
}
