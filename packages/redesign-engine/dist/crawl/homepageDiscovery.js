...
function isHomeLabel(text) {
    if (!text)
        return false;
    const t = text.toLowerCase().trim();
    if (HOME_LABELS.has(t))
        return true;
...
function urlContainsGeneric(url) {
...
export function discoverHomepage(pages, baseUrl, navigation, originRoot, warnings) {
    const candidates = new Map();
    function addScore(url, score, reason, page, index = -1) {
    ...
    pages.forEach((p, i) => {
        if (isHomeUrl(p.url))
            addScore(p.url, 25, 'URL is origin root or /index.html', p, i);
        if (p.canonicalUrl && isHomeUrl(p.canonicalUrl))
            addScore(p.url, 15, 'rel=canonical points to root', p, i);
        if (p.logoHref && isHomeUrl(p.logoHref))
            addScore(p.url, 20, 'logo/home link points to root', p, i);
        if (p.path === 'index')
            addScore(p.url, 5, 'slug is index', p, i);
        if (p.h1 && !isGeneric(p.h1) && p.h1.length > 1)
            addScore(p.url, 5, 'H1 is not generic', p, i);
        if (p.title && !isGeneric(p.title) && p.title.length > 1)
            addScore(p.url, 5, 'title is not generic', p, i);
        if (p.metaDescription && p.metaDescription.length > 10)
            addScore(p.url, 3, 'meta description present', p, i);
        if (isGeneric(p.h1) || isGeneric(p.title))
            addScore(p.url, -25, 'H1/title is generic page name', p, i);
        if (urlContainsGeneric(p.url))
            addScore(p.url, -20, 'URL contains generic page keyword', p, i);
    });
    ...
    for (const n of flatNav) {
        if (!n.url)
            continue;
        if (isHomeLabel(n.label))
            addScore(n.url, 20, `nav label "${n.label}" indicates home`);
        if (n.source === 'header')
            addScore(n.url, 5, 'linked from header');
        if (n.source === 'sitemap')
            addScore(n.url, 2, 'linked from sitemap');
    }
