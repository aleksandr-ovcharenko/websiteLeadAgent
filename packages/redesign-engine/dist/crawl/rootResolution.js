...
export function rootVariants(inputUrl) {
    let u;
    try {
 ...
 */
export async function resolveSiteRoot(inputUrl, opts, fetchRoot) {
    ...
    for (const variant of variants) {
        for (let attempt = 0; attempt <= retries; attempt++) {
            ...
            try {
                ...
                const isTimeout = err instanceof RootFetchTimeout || /timeout/i.test(String(err?.message));
                lastStatus = isTimeout ? 'TIMEOUT' : 'UNREACHABLE';
                lastReason = isTimeout ? `timeout after ${timeoutMs}ms` : String(err?.message || err);
                last = {
                    requestedUrl: variant,
 ...
 */
export function resolvedHomepageUrl(res) {
    if (res.homepageStatus !== 'FOUND' || !res.finalUrl)
        return undefined;
