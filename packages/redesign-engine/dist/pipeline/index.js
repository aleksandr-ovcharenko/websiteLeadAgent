...
export async function runCrawl(options) {
    ...
    const emit = async (level, eventType, message, details) => {
        if (onActivity) {
            try {
                await onActivity({ module: 'FACTORY', eventType, message, details, level });
            }
            catch { /* no-op */ }
        }
    };
    ...
    const activeRun = await prisma.redesignRun.findFirst({
        where: {
            leadId: l.id,
    ...
    try {
        ...
        await prisma.redesignRun.update({
            ...
            data: {
                crawlJsonPath,
                homepageCandidate: crawlResult.homepage,
                currentCrawl: { homepage: crawlResult.homepage, pageCount: crawlResult.pages.length, warnings: crawlResult.warnings },
                stage: 'CRAWL_READY'
            }
        });
        await prisma.lead.update({
...
export async function generateSite(options) {
    ...
    if (options.crawlRunId) {
        ...
        if (!existingRun)
            throw new Error(`Crawl run not found: ${options.crawlRunId}`);
        if (existingRun.leadId !== l.id)
            throw new Error(`Crawl run ${options.crawlRunId} does not belong to lead ${l.id}`);
        ...
        if (!existingRun.crawlJsonPath)
            throw new Error(`Crawl run ${options.crawlRunId} has no crawl artifact`);
        crawlJsonPath = existingRun.crawlJsonPath;
        const raw = await readFile(crawlJsonPath, 'utf8');
        crawlResult = JSON.parse(raw);
        run = existingRun;
        if (options.force) {
            await prisma.redesignRun.update({
                where: { id: run.id },
    ...
    try {
        ...
        await prisma.redesignRun.update({
            where: { id: run.id },
            data: {
                currentCrawl: { homepage: crawlResult.homepage, pageCount: crawled.length, warnings: crawlResult.warnings },
        ...
        if (options.semanticOnly) {
            await prisma.redesignRun.update({
                where: { id: run.id },
                data: { currentCrawl: { homepage: crawlResult.homepage, pageCount: crawled.length, warnings: crawlResult.warnings, sourceContentGraphPath, sourceDocumentsJsonPath }, stage: 'CONTENT_EXTRACTED' }
            });
        ...
        await prisma.site.update({
            where: { id: siteId },
            data: { domain, status: 'ACTIVE', settings: { previewUrl: `http://localhost:3000/showcase/${previewSlug}` } }
        });
        await prisma.redesignRun.update({
            where: { id: run.id },
        ...
        const siteBuild = await prisma.siteBuild.create({
            data: {
                siteId,
                demoVariantId,
                templateId,
        ...
        const snapshotId = await ensureDependencySnapshot(prisma, templateId, process.cwd(), { commitSha: process.env.GIT_SHA }).catch((err) => {
